 ...
 * representation, so each surface can persist whichever its host supports (Excel → workbook
 * settings as JSON; Word → a custom XML part). It NEVER touches Office.js, so it's unit-tested.
 */
